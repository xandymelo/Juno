...
from enums.vinculo import Vinculo
...
usuarioID = [random.randint(10000, 999999) for _ in range(QTD_DADOS)] 
sigaaID = [random.randint(QTD_DADOS, 999999) for _ in range(QTD_DADOS)] 
enderecoID = [str(random.randint(QTD_DADOS, 999999))for _ in range(QTD_DADOS)]
nome = [fake.name() for _ in range(QTD_DADOS)]
sobrenome = [fake.last_name() for _ in range(QTD_DADOS)]  # Gera sobrenomes fictícios
cpf = [fake.cpf() for _ in range(QTD_DADOS)]
senha = [fake.password() for _ in range(QTD_DADOS)]
vinculo = [random.randint(1, len(list(Vinculo))) for _ in range(QTD_DADOS)]
telefone = ['81' + fake.numerify('#######') for _ in range(QTD_DADOS)]  # Gera telefones fictícios com DDD 81
email = [f"{n.lower().replace(' ', '')}.{s.lower().replace(' ', '')}@ufrpe.br" for n, s in zip(nome, sobrenome)]  # Gera emails fictícios
nascimento = [fake.date_of_birth(minimum_age=17, maximum_age=70).strftime('%d/%m/%Y') for _ in range(QTD_DADOS)]  # Gera datas de nascimento fictícias com pelo menos 17 anos
...
veiculoID = veiculoID = [str(random.randint(QTD_DADOS, 999999)) for _ in range(QTD_DADOS)]
usuarioID_veiculo = [random.choice(usuarioID) if random.random() < 0.8 else '-' for _ in range(QTD_DADOS)]  # Atribuindo IDs de usuários aleatoriamente aos veículos (80% de chance de ter um usuário)
tipo = [random.choice(['carro', 'moto']) for _ in range(QTD_DADOS)]
...
turno = [random.choice(["Matutino", "Vespertino"]) for _ in range(QTD_DADOS)]
periodo = [random.randint(1, 10) for _ in range(QTD_DADOS)]
...
usuarioID = usuarioID
veiculoID = veiculoID
horaSaida = [(datetime.strptime("4:30", "%H:%M") + timedelta(minutes=random.randint(0, 1050))).strftime("%H:%M:%S") for _ in range(QTD_DADOS)]
origem = [random.choice(municipio) + ", " + random.choice(bairro) + ", " + random.choice(rua) + ", " + str(random.choice(numero)) + ", " + str(random.choice(complemento)) for _ in range(QTD_DADOS)]
status = [random.choice(["Ocorrendo", "ocorreu", "Ocorrerá"]) for _ in range(QTD_DADOS)]
vagasDisponiveis = []
for s in status:
    if s == "Ocorrerá":
...
departamentos = ["Departamento de Zootecnia", "CEGOE", "Departamento de Educação Física", "Biblioteca Setorial",  "DMV", "EspaçoTec", "Reitoria",
                 ...
                 "CEAGRE II"]
destino = [random.choice(departamentos) for _ in range(QTD_DADOS)]
